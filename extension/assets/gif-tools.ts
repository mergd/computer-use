/**
 * gif-tools.ts - GIF recording functionality
 *
...
// =============================================================================
// GIF Frame Storage Types
// =============================================================================

/**
 * Group data stored in the GifFrameStorage
 */
...
/**
 * GifFrameStorage class (xe) - Manages GIF frames per tab group
 */
class GifFrameStorageClass {
  private storage: Map<number, GifGroupData> = new Map();
  private recordingGroups: Set<number> = new Set();
...
// Singleton instance
export const gifFrameStorage = new GifFrameStorageClass();
...
/**
 * Action delay map type
 */
type ActionDelayMap = {
  [key in ComputerAction | "navigate"]?: number;
};
...
export function getActionDelay(actionType: string): number {
  ...
  return delays[actionType as keyof ActionDelayMap] ?? 800;
}
 ...
 */
async function handleStartRecording(groupId: number): Promise<ToolResultSuccess> {
 ...
 */
async function handleStopRecording(groupId: number): Promise<ToolResultSuccess> {
  const wasRecording = gifFrameStorage.isRecording(groupId);
...
/**
 * Frame data prepared for GIF generation
 */
interface FrameDataForGeneration {
  base64: string;
  format: string;
  action: GifActionInfo | undefined;
  delay: number;
...
async function handleExport(
  ...
  const frameData: FrameDataForGeneration[] = frames.map((frame) => ({
    base64: frame.base64,
    format: "png",
    action: frame.action,
  ...
  const gifResult = await new Promise<GifGenerationResult>((resolve, reject) => {
    chrome.runtime.sendMessage(
      ...
      (response: { success: boolean; result?: GifGenerationResult; error?: string }) => {
  ...
  if (shouldDownload) {
    ...
    const uploadOutput = uploadResult[0].result as { output: string };
 ...
 * Handle clear action
 */
async function handleClear(groupId: number): Promise<ToolResultSuccess> {
  const frameCount = gifFrameStorage.getFrameCount(groupId);
...
export const gifCreatorTool: ToolDefinition = {
  ...
  execute: async (params: Record<string, unknown>, context: ToolExecutionContext): Promise<ToolResult> => {
    try {
      const args = params as unknown as GifCreatorParams;
