...
export const uploadImageTool = {
    ...
    execute: async (params, context) => {
        try {
            ...
            const imageData = findImageInMessages(context.messages, args.imageId);
            if (!imageData) {
                return {
            ...
            const results = await chrome.scripting.executeScript({
                target: { tabId: tab.id },
                func: (elementRef, coordinate, base64Data, filename) => {
                    try {
                        let target = null;
                        if (coordinate) {
...
export const readConsoleMessagesTool = {
    ...
    execute: async (params, context) => {
        try {
            const { tabId, onlyErrors = false, clear = false, pattern, limit = 100 } = params;
            if (!context?.tabId)
                throw new Error("No active tab found");
            ...
            const messages = re.getConsoleMessages(tab.id, onlyErrors, pattern);
            if (clear)
                re.clearConsoleMessages(tab.id);
...
export const readNetworkRequestsTool = {
    ...
    execute: async (params, context) => {
        try {
            const { tabId, urlPattern, clear = false, limit = 100 } = params;
            if (!context?.tabId)
                throw new Error("No active tab found");
            const effectiveTabId = await K.getEffectiveTabId(tabId, context.tabId);
            const tab = await chrome.tabs.get(effectiveTabId);
            ...
            const requests = re.getNetworkRequests(tab.id, urlPattern);
            if (clear)
                re.clearNetworkRequests(tab.id);
            if (requests.length === 0) {
                let reqType = "network requests";
                if (urlPattern)
                    reqType = `requests matching "${urlPattern}"`;
                return {
...
const turnAnswerStartSchema = { type: "object", properties: {}, required: false };
export const turnAnswerStartTool = {
 ...
 */
async function launchShortcutWindow(params) {
    ...
    const sidepanelUrl = chrome.runtime.getURL(`sidepanel.html?mode=window&sessionId=${sessionId}${skipPermissions ? "&skipPermissions=true" : ""}${model ? `&model=${encodeURIComponent(model)}` : ""}`);
...
export const shortcutsListTool = {
    ...
    execute: async () => {
        try {
            ...
            const shortcuts = prompts.map((prompt) => ({
                id: prompt.id,
            ...
            return {
                output: JSON.stringify({ message: `Found ${shortcuts.length} shortcut(s)`, shortcuts }, null, 2),
            };
        }
        catch (err) {
            return {
