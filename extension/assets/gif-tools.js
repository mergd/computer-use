...
async function handleExport(params, tab, groupId, context) {
    ...
    const gifResult = await new Promise((resolve, reject) => {
        chrome.runtime.sendMessage({ type: "GENERATE_GIF", frames: frameData, options }, (response) => {
    ...
    execute: async (params, context) => {
        try {
            ...
            switch (args.action) {
                ...
                default:
                    throw new Error(`Unknown action: ${args.action}. Must be one of: start_recording, stop_recording, export, clear`);
            }
        }
        catch (err) {
            return {
