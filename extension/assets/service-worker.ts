...
/** Native messaging port type */
type NativePort = chrome.runtime.Port | null;

/** Native host status response */
interface NativeHostStatus {
  nativeHostInstalled: boolean;
  mcpConnected: boolean;
}

/** Native message types from host */
...
/** Enhanced tool response for native messaging */
...
/** MCP notification for native messaging */
...
/** Message sender with tab info */
interface MessageSenderWithTab extends chrome.runtime.MessageSender {
  tab?: chrome.tabs.Tab;
}
...
// Declare global skipPermissions flag
declare global {
...
let nativePort: NativePort = null;
...
const mainTabAlivenessCache = new Map<number, MainTabAlivenessCacheEntry>();
...
async function handleNativeMessage(message: NativeMessage): Promise<void> {
  switch (message.type) {
    ...
    case "set_skip_permissions":
      (self as unknown as ServiceWorkerGlobalScope).__skipPermissions = !!message.value;
      break;
    ...
    if (method === "execute_tool") {
      ...
      if (toolName === "clipboard_read") {
        try {
          ...
          const response = await chrome.runtime.sendMessage({ type: "CLIPBOARD_READ" }) as {
            success: boolean;
            text?: string;
            error?: string;
          };
      ...
      if (toolName === "clipboard_write") {
        try {
          ...
          const response = await chrome.runtime.sendMessage({
            ...
            text: (params.args?.text as string) || ""
      ...
      if (toolName === "set_cookie") {
        try {
          ...
          if (params.args?.domain) opts.domain = params.args.domain as string;
          if (params.args?.path) opts.path = params.args.path as string;
          if (params.args?.secure) opts.secure = params.args.secure as boolean;
          if (params.args?.httpOnly) opts.httpOnly = params.args.httpOnly as boolean;
          if (params.args?.expirationDate)
            opts.expirationDate = params.args.expirationDate as number;
      ...
      if (toolName === "search_history") {
        try {
          const opts: chrome.history.HistoryQuery = {
            text: (params.args?.query as string) || "",
            maxResults: (params.args?.maxResults as number) || 100,
          };
          if (params.args?.startTime) opts.startTime = params.args.startTime as number;
          if (params.args?.endTime) opts.endTime = params.args.endTime as number;
...
function enhancePermissionDenialMessage(
  ...
  let enhancedContent: string | Array<{ type: string; text: string }>;
...
function sendToolResponse(
  ...
  let nativeResponse: NativeToolResponse;
  if (is_error) {
...
async function handleTabSwitchUrl(
  ...
  try {
    ...
    const group = await TabGroupManager.findGroupByTab(targetTabId) as TabGroup & { isUnmanaged?: boolean } | null;
...
async function ensureTabInManagedGroup(tabId: number): Promise<void> {
  await TabGroupManager.initialize(true);
  const group = await TabGroupManager.findGroupByTab(tabId) as TabGroup & { isUnmanaged?: boolean; chromeGroupId?: number } | null;
...
async function ensureOffscreenDocument(): Promise<void> {
  ...
  try {
    ...
    if ((error as Error).message?.includes("Only a single offscreen document")) {
      offscreenDocumentCreated = true;
      return;
    }
    throw error;
...
interface PopulateInputMessage {
  prompt?: string;
  ...
  try {
    ...
    const checkTabForMainTab = async (index: number): Promise<void> => {
      ...
      chrome.runtime.sendMessage(
        ...
        async (response: { success?: boolean } | undefined) => {
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  (async () => {
    ...
    if (message.type === "STATIC_INDICATOR_HEARTBEAT") {
      await handleStaticIndicatorHeartbeat(sender as MessageSenderWithTab, sendResponse);
      return;
    }

    // Handle dismissing static indicator for a group
    if (message.type === "DISMISS_STATIC_INDICATOR_FOR_GROUP") {
      await handleDismissStaticIndicator(sender as MessageSenderWithTab, sendResponse);
      return;
    }
  })();
