/**
 * computer-tool.js - Computer tool for browser automation
 *
 * Contains the main computer tool (ie) and helper functions:
 * - oe: coordinate scaling
 * - ne: scroll helper
 * - ae: get element coordinates from ref
 * - se: click action handler
 * - ce: screenshot capture
 * - ue: get scroll position
 */
...
// Scale coordinates based on viewport vs screenshot dimensions (oe)
...
// Scroll at coordinates using scripting API (ne)
export async function scrollAtCoordinates(tabId, x, y, deltaX, deltaY) {
...
// Get element coordinates from ref ID (ae)
export async function getElementCoordinates(tabId, ref) {
  try {
...
function parseModifiers(modifierString) {
  const modifierMap = {
    alt: 1,
    ctrl: 2,
    control: 2,
  ...
  let mask = 0;
  for (const mod of modifierString) {
    mask |= modifierMap[mod] || 0;
  }
  return mask;
}

// Extract modifier keys from string
function extractModifiers(modifierString) {
...
// Click action handler (se)
export async function handleClick(tabId, params, clickCount = 1, originalUrl) {
  let x, y;
...
// Get scroll position (ue)
export async function getScrollPosition(tabId) {
  const results = await chrome.scripting.executeScript({
    target: { tabId },
    func: () => ({
...
function getPermissionType(action) {
  const permissionMap = {
...
async function captureScrollScreenshot(tabId, permissionManager) {
  try {
...
// Type action handler
async function handleType(tabId, params, originalUrl) {
  if (!params.text) {
...
async function handleScroll(tabId, params, permissionManager) {
...
// Key action handler
async function handleKey(tabId, params, originalUrl) {
  if (!params.text) {
...
// Drag action handler
async function handleDrag(tabId, params, originalUrl) {
...
// Zoom action handler
async function handleZoom(tabId, params) {
...
async function handleScrollTo(tabId, params, originalUrl) {
  if (!params.ref) {
...
async function handleHover(tabId, params, originalUrl) {
  let x, y;
