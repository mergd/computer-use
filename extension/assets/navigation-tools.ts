...
// =============================================================================
// Local Type Definitions (not in shared types)
// =============================================================================

/** Parameters for tabs_context tool (empty) */
interface TabsContextParams {}

/** Parameters for tabs_create tool (empty) */
interface TabsCreateParams {}

/** Parameters for tabs_context_mcp tool */
interface TabsContextMcpParams {
  createIfEmpty?: boolean;
}

/** Parameters for tabs_create_mcp tool (empty) */
interface TabsCreateMcpParams {}

/** MCP Tab Context returned from getOrCreateMcpTabContext */
interface McpTabContextResult {
  currentTabId: number;
...
/**
 * Tool definition interface with generic parameter type
 */
interface Tool<TParams = Record<string, unknown>> {
  ...
  toAnthropicSchema: (context?: ToolExecutionContext) => Promise<AnthropicToolSchema>;
}
...
export const navigateTool: Tool<NavigateParams> = {
  name: "navigate",
  description:
  ...
  execute: async (params: NavigateParams, context: ToolExecutionContext): Promise<ToolResult> => {
    try {
      ...
      if (url && !["back", "forward"].includes(url.toLowerCase())) {
        try {
          const category = await DomainCategoryCache.getCategory(url) as DomainCategory;
          if (
            category &&
...
export const tabsContextTool: Tool<TabsContextParams> = {
  name: "tabs_context",
  description: "Get context information about all tabs in the current tab group",
  parameters: {},
  execute: async (_params: TabsContextParams, context: ToolExecutionContext): Promise<ToolResult> => {
    try {
      if (!context?.tabId) throw new Error("No active tab found");
...
export const tabsCreateTool: Tool<TabsCreateParams> = {
  name: "tabs_create",
  description: "Creates a new empty tab in the current tab group",
  parameters: {},
  execute: async (_params: TabsCreateParams, context: ToolExecutionContext): Promise<ToolResult> => {
    try {
      if (!context?.tabId) throw new Error("No active tab found");
...
export const tabsContextMcpTool: Tool<TabsContextMcpParams> = {
  ...
  execute: async (params: TabsContextMcpParams): Promise<ToolResult> => {
    try {
...
export const tabsCreateMcpTool: Tool<TabsCreateMcpParams> = {
  ...
  execute: async (): Promise<ToolResult> => {
    try {
      ...
      const availableTabs: TabMetadata[] = groupTabs
