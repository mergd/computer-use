...
import { AccessibilityTreeResult, ClaudeElementMap } from "./types.js";
...
/**
 * Filter modes for accessibility tree generation
 */
 ...
 */
const ROLE_MAP: Record<string, string | ((element: Element) => string)> = {
  a: "link",
  button: "button",
 ...
 */
function shouldIncludeElement(element: Element, options: TreeOptions): boolean {
  const tagName = element.tagName.toLowerCase();
...
(function () {
  ...
  window.__generateAccessibilityTree = function (
    ...
    try {
       ...
       */
      function buildTree(element: Element, currentDepth: number, treeOptions: TreeOptions): void {
        ...
        const shouldInclude =
          shouldIncludeElement(element, treeOptions) ||
        ...
        if (shouldInclude) {
          ...
          if (element.tagName.toLowerCase() === "select") {
            const selectOptions = (element as HTMLSelectElement).options;
      ...
      if (maxChars != null && pageContent.length > maxChars) {
        let errorMessage =
          "Output exceeds " + maxChars + " character limit (" + pageContent.length + " characters). ";
