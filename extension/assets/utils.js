 ...
 */
export function assertNotRestrictedTab(tab) {
...
export const coerceParameterTypes = (toolName, params, tools) => {
    ...
    if (!tool || !tool.parameters || typeof params !== "object" || !params)
        return params;
...
export function findImageInMessages(messages, imageId) {
    ...
    for (let i = messages.length - 1; i >= 0; i--) {
        ...
        if (msg.role === "user" && Array.isArray(msg.content)) {
            ...
            const textIndex = msg.content.findIndex((c) => c.type === "text" && c.text?.includes(imageId));
            if (textIndex !== -1) {
                console.info(`[imageUtils] Found image ID in user text at index ${textIndex}, looking for next adjacent image`);
                for (let j = textIndex + 1; j < msg.content.length; j++) {
                    const item = msg.content[j];
                    if (item.type === "image") {
                        const imgItem = item;
                        if (imgItem.source && "data" in imgItem.source && imgItem.source.data) {
                            console.info(`[imageUtils] Found user-uploaded image for ID ${imageId} at index ${j}`);
