...
/**
 * TabGroupManager interface for dependency injection
 */
...
/**
 * Resize parameters for screenshot processing
 */
...
/**
 * Key definition for keyboard input
 */
 ...
 */
interface ConsoleMessageStorage {
  domain: string;
  messages: ConsoleMessage[];
}

/**
 * Network request with request ID
 */
interface NetworkRequestWithId extends NetworkRequest {
  requestId: string;
}
 ...
 */
interface NetworkRequestStorage {
  domain: string;
  requests: NetworkRequestWithId[];
}

/**
 * CDP Runtime.consoleAPICalled event args
 */
interface ConsoleAPICalledArgs {
  type?: string;
  args?: Array<{
    value?: unknown;
    description?: string;
...
/**
 * CDP Runtime.exceptionThrown event args
 */
interface ExceptionThrownArgs {
  exceptionDetails?: {
    exception?: {
      description?: string;
    };
...
/**
 * CDP Network.requestWillBeSent event args
 */
interface RequestWillBeSentArgs {
  requestId: string;
  request: {
    url: string;
    method: string;
...
/**
 * CDP Network.responseReceived event args
 */
interface ResponseReceivedArgs {
  requestId: string;
  response: {
    status: number;
  };
...
/**
 * CDP Network.loadingFailed event args
 */
interface LoadingFailedArgs {
  requestId: string;
}

/**
 * Viewport info result from script execution
 */
...
declare global {
  ...
  var __cdpConsoleMessagesByTab: Map<number, ConsoleMessageStorage>;
  var __cdpNetworkRequestsByTab: Map<number, NetworkRequestStorage>;
  var __cdpNetworkTrackingEnabled: Set<number>;
  var __cdpConsoleTrackingEnabled: Set<number>;
  var __cdpDebuggerEventHandler: ((
    source: { tabId?: number },
    method: string,
    params: unknown
  ) => void) | undefined;
...
let tabGroupManagerRef: TabGroupManager | null = null;

/**
 * Set the TabGroupManager dependency
 * @param tabGroupManager - The TabGroupManager singleton (K from mcp-tools.js)
 */
export function setTabGroupManager(tabGroupManager: TabGroupManager): void {
 ...
 */
function ceilingDivision(dividend: number, divisor: number): number {
 ...
 */
function calculateTokensForDimensions(width: number, height: number, pxPerToken: number): number {
 ...
 */
function calculateTargetDimensions(width: number, height: number, params: ResizeParams): [number, number] {
...
class ScreenshotContextClass {
  private contexts: Map<number, ViewportContext> = new Map();
  ...
  getContext(tabId: number): ViewportContext | undefined {
...
class CDPDebugger {
  ...
  static get consoleMessagesByTab(): Map<number, ConsoleMessageStorage> {
    return globalThis.__cdpConsoleMessagesByTab;
  }

  static get networkRequestsByTab(): Map<number, NetworkRequestStorage> {
  ...
  registerDebuggerEventHandlers(): void {
    ...
    globalThis.__cdpDebuggerEventHandler = (
      ...
      if (method === "Runtime.consoleAPICalled") {
        const args = params as ConsoleAPICalledArgs;
      ...
      if (method === "Runtime.exceptionThrown") {
        const args = params as ExceptionThrownArgs;
      ...
      if (method === "Network.requestWillBeSent") {
        const args = params as RequestWillBeSentArgs;
        const request: NetworkRequestWithId = {
      ...
      if (method === "Network.responseReceived") {
        const args = params as ResponseReceivedArgs;
        const storage = CDPDebugger.networkRequestsByTab.get(tabId);
        if (storage) {
          const request = storage.requests.find(
            (req) => req.requestId === args.requestId
          );
          if (request) {
      ...
      if (method === "Network.loadingFailed") {
        const args = params as LoadingFailedArgs;
        const storage = CDPDebugger.networkRequestsByTab.get(tabId);
        if (storage) {
          const request = storage.requests.find(
            (req) => req.requestId === args.requestId
  ...
  extractDomain(url: string | undefined): string {
  ...
  addConsoleMessage(
    ...
    if (storage.messages.length > 0) {
      const lastTimestamp =
        storage.messages[storage.messages.length - 1].timestamp;
  ...
  async screenshot(
    ...
    try {
      ...
      const { width: viewportWidth, height: viewportHeight, devicePixelRatio } =
        scriptResult[0].result as ViewportInfo;

      const captureResult = await this.sendCommand<{ data: string }>(
        tabId,
