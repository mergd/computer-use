 ...
 *
 * Generates an accessibility tree representation of the page for browser automation.
 * This module traverses the DOM and builds a text-based accessibility tree that can
 * be used by AI models to understand page structure and interact with elements.
 */
...
/** Filter mode for accessibility tree generation */
type FilterMode = "all" | "interactive";

/** Options for tree traversal and filtering */
interface TraversalOptions {
  filter: FilterMode;
  refId: string | null;
}

/** Result object returned by the accessibility tree generator */
...
/** Map of tag names to their implicit ARIA roles */
interface RoleMap {
    ...
    try {
      ...
      function getAccessibleName(element: Element): string {
        ...
        for (let childIndex = 0; childIndex < element.childNodes.length; childIndex++) {
          ...
          if (childNode.nodeType === Node.TEXT_NODE) {
            collectedText += childNode.textContent;
          }
        }
       ...
       * @param element - The DOM element
       * @param traversalOptions - Filter options
       * @returns True if the element should be included
       */
      function shouldIncludeElement(element: Element, traversalOptions: TraversalOptions): boolean {
        const tagName = element.tagName.toLowerCase();
        ...
        if (traversalOptions.filter !== "all" && element.getAttribute("aria-hidden") === "true") {
          return false;
        }

        // Skip invisible elements unless filter is "all"
        if (traversalOptions.filter !== "all" && !isElementVisible(element)) {
          return false;
        }

        // Skip elements outside viewport unless filter is "all" or we're focused on a ref
        if (traversalOptions.filter !== "all" && !traversalOptions.refId) {
        ...
        if (traversalOptions.filter === "interactive") {
       ...
       * @param traversalOptions - Filter options
       */
      function buildTree(element: Element, currentDepth: number, traversalOptions: TraversalOptions): void {
        ...
        const shouldInclude = shouldIncludeElement(element, traversalOptions) || (traversalOptions.refId !== null && currentDepth === 0);
        ...
        if (element.children && currentDepth < depthLimit) {
          for (let childIndex = 0; childIndex < element.children.length; childIndex++) {
            ...
            buildTree(element.children[childIndex], nextDepth, traversalOptions);
          }
        }
      }
      ...
      const depthLimit: number = maxDepth != null ? maxDepth : 15;
      ...
      if (refId) {
        ...
        buildTree(targetElement, 0, traversalOptions);
      } else {
        // Start from document body
        if (document.body) {
          buildTree(document.body, 0, traversalOptions);
        }
      }
...
// Content script - types only (runtime attaches to window)
export type { FilterMode, TraversalOptions, AccessibilityTreeResult };
