 ...
 *
 * This module handles visual indicators shown during browser automation:
 * - Pulsing glow border around the viewport when agent is active
 * - Stop button to allow users to halt agent execution
 * - Static indicator showing "Claude is active in this tab group"
 */
...
interface RuntimeMessage {
  type: 'STOP_AGENT' | 'SWITCH_TO_MAIN_TAB' | 'DISMISS_STATIC_INDICATOR_FOR_GROUP' | 'STATIC_INDICATOR_HEARTBEAT';
  fromTabId?: string;
}
...
/** Message types received from the background script */
...
let glowBorderElement: HTMLDivElement | null = null;
let stopButtonContainer: HTMLDivElement | null = null;
let staticIndicatorContainer: HTMLDivElement | null = null;
let isAgentIndicatorActive: boolean = false;
let isStaticIndicatorActive: boolean = false;
let wasAgentIndicatorVisible: boolean = false;
let wasStaticIndicatorVisible: boolean = false;
let heartbeatIntervalId: ReturnType<typeof setInterval> | null = null;
let isMcpMode: boolean = false;
 ...
 */
function createGlowBorderElement(): HTMLDivElement {
 ...
 */
function createStopButtonContainer(): HTMLDivElement {
 ...
 */
function createStaticIndicatorContainer(): HTMLDivElement {
  const container = document.createElement("div");
  ...
  const chatButton = container.querySelector("#claude-static-chat-button") as HTMLButtonElement | null;
  const chatTooltip = container.querySelector("#claude-static-chat-tooltip") as HTMLSpanElement | null;
  ...
  const closeButton = container.querySelector("#claude-static-close-button") as HTMLButtonElement | null;
  const closeTooltip = container.querySelector("#claude-static-close-tooltip") as HTMLSpanElement | null;
...
function showAgentIndicators(): void {
  ...
  requestAnimationFrame(() => {
    ...
    if (stopButtonContainer) {
      const stopButton = stopButtonContainer.querySelector("#claude-agent-stop-button") as HTMLButtonElement | null;
      if (stopButton) {
...
/**
 * Initializes the agent visual indicator system
 * Sets up message listeners and cleanup handlers
 */
function initializeAgentVisualIndicator(): void {
...
// Auto-initialize when the script loads (wrapped in IIFE for isolation)
(function () {
  initializeAgentVisualIndicator();
})();

// Content script - types only (runtime uses message passing)
export type { RuntimeMessage, RuntimeResponse, IndicatorMessage };
